...
const MovieCard = (props: MovieCardProps) => {
    ...
    return (
        <Link
            href={`/movie/${id}`}
            className='bg-white rounded-md min-h-[300px] p-4 relative flex flex-col gap-3 cursor-pointer hover:opacity-50 transition-all opacity-100 group overflow-hidden'
        >
            <div
                className={cn(
                    'group-hover:top-0 group-hover:right-0 group-hover:left-0 group-hover:bottom-0',
                    '-z-10 group-hover:z-10 z',
                    'bg-black',
                ...
                style={{
                    backgroundImage: `url(${process.env.NEXT_PUBLIC_API_IMAGE_PATH}/${backdrop_path}) no-repeat`,
            ...
            <div className='relative pb-8'>
                ...
                <Link href={`/movie/${id}`} className='absolute bottom-0 right-0 text-gray-950 group-hover:font-semibold'>Read more</Link>
            </div>
        </Link>
    )
}
